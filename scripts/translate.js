 ...
 *
 * Usage: node scripts/translate.js <slug> <language> [--title "翻译标题"] [--subtitle "翻译副标题"] [--content "翻译内容HTML"]
 * Or via stdin: echo '{"slug":"xxx","title":"...","subtitle":"...","content":"..."}' | node scripts/translate.js
 */
...
function saveTranslation(slug, lang, title, content, subtitle) {
  ...
  if (subtitle != null && subtitle !== '') {
    articleMeta.translatedSubtitle = subtitle;
  }
...
if (args.length >= 1 && args[0] !== '--stdin') {
  ...
  let title = null, content = null, subtitle = null;
  for (let i = 2; i < args.length; i++) {
    if (args[i] === '--title') title = args[++i];
    if (args[i] === '--subtitle') subtitle = args[++i];
    if (args[i] === '--content') content = args[++i];
  }
  const result = saveTranslation(slug, lang, title, content, subtitle);
  ...
  process.stdin.on('end', () => {
    try {
      ...
      const result = saveTranslation(data.slug, data.lang || 'zh', data.title, data.content, data.subtitle);
      console.log(JSON.stringify(result));
    } catch (e) {
